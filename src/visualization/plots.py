...
def plot_target_distribution(df, target_col, fig_dir):
    ...
    axes[0].set_xlabel("Mức độ"); axes[0].set_ylabel("Số lượng")
    ...
    axes[1].set_title("Target nhị phân", fontweight="bold"); axes[1].set_ylabel("Số lượng")
...
def plot_categorical(df, cat_cols, target_col, fig_dir):
    ...
    for i, col in enumerate(cat_cols):
        ...
        axes[i].legend(title="Target", fontsize=8); axes[i].tick_params(axis="x", rotation=30)
...
def plot_violin_by_target(df, num_cols, target_col, fig_dir):
    """Violin plot biến số theo target."""
    temp = df.copy()
    ...
    for i, col in enumerate(num_cols[:n]):
        sns.violinplot(data=temp, x="target_label", y=col, ax=axes[i],
                       hue="target_label", palette={"Không bệnh": "#2ecc71", "Có bệnh": "#e74c3c"},
                       inner="box", legend=False)
        axes[i].set_title(f"{col}", fontweight="bold"); axes[i].set_xlabel("")
    for j in range(i+1, len(axes)):
        axes[j].set_visible(False)
...
def plot_model_comparison(comp_df, fig_dir):
    """Bar chart so sánh metric giữa các mô hình."""
...
def plot_confusion_matrices(preds, y_test, fig_dir):
    """Confusion matrix cho top-4 mô hình."""
...
def plot_pr_curves(model_results, preds, X_test, y_test, fig_dir):
    """Precision-Recall curve cho tất cả mô hình."""
    fig, ax = plt.subplots(figsize=(10, 8))
    colors = plt.cm.Set1(np.linspace(0, 1, len(model_results)))
    for (name, _), color in zip(model_results.items(), colors):
        yp = preds[name]["y_proba"]
